...
class Wage {
    ...
    isWeekend() {
        ...
        return this.day === "Saturday" || this.day === "Sunday"  // Kortare if-else statement
    }
    calculateWeekDayWage() {
        this.normalSalary = this.getTotalHours() * this.hourlySalary
        return this.extraSalary = this.getExtraTimeHours() * this.hourlySalary * 0.5
        
    }
    calculateWeekendWage() {
        this.normalSalary = this.getTotalHours() * this.hourlySalary
        return this.weekendSalary = this.normalSalary
    }
    calculateWage() {
        if(this.isWeekend()){
    ...
    getMessage() {
        if(this.getTotalHours() > this.maximumHours) {
            ...
            return `Your total salary is: ${this.calculateWeekDayWage()}`
        }
...
const wage3 = new Wage("Tuseday", 8, 22, 200)
